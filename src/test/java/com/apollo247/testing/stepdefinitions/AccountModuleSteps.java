...
public class AccountModuleSteps {
    ...
    public void family_member_should_be_created_successfully() {
        Assert.assertTrue(
            Pages.manageFamilyPage.isSuccessToastDisplayed(),
            "❌ Family member creation success message not displayed"
        );
    }
    ...
    public void appointments_section_should_be_displayed() {
        Assert.assertTrue(
            Pages.myappointmentsPage.isAppointmentsPageDisplayed(),
            "❌ Appointments page not displayed"
        );
    ...
    public void appointments_section_should_still_be_displayed() {
        Assert.assertTrue(
            Pages.myappointmentsPage.isPageLoadedAfterRefresh(),
            "❌ Appointments page not visible after refresh"
        );
    ...
    public void corporate_benefits_error_message_should_be_displayed() {
        Assert.assertTrue(
            Pages.membershipsPage.isCorporateErrorDisplayed(),
            "❌ Corporate error not shown"
        );
        System.out.println("✅ Corporate error verified: " +
            Pages.membershipsPage.getCorporateErrorText());
    }
    ...
    public void the_following_plan_details_should_be_visible_on_the_page(
        ...
        for (int i = 1; i < rows.size(); i++) {
            Assert.assertTrue(
                Pages.membershipsPage.isPlanDetailVisible(rows.get(i)),
                "❌ Expected detail not found: '" + rows.get(i) + "'"
            );
    ...
    public void push_notifications_toggle_should_be_active() {
        Assert.assertTrue(
            Pages.notificationsPage.isPushToggleVisible(),
            "❌ Push Notifications toggle not visible"
        );
    ...
    public void sms_notifications_toggle_should_be_active() {
        Assert.assertTrue(
            Pages.notificationsPage.isSmsToggleVisible(),
            "❌ SMS Notifications toggle not visible"
        );
    ...
    public void all_selected_notifications_should_be_enabled() {
        Assert.assertTrue(
            Pages.notificationsPage.areAllTogglesVisible(),
            "❌ Notification toggles not visible"
        );
    ...
    public void all_help_categories_should_be_visible() {
        Assert.assertTrue(
            Pages.needHelpPage.areHelpCategoriesVisible(),
            "❌ Help categories not displayed"
        );
    ...
    public void medicines_help_page_should_be_loaded() {
        Assert.assertTrue(
            Pages.needHelpPage.isMedicinesPageLoaded(),
            "❌ Medicines page not loaded"
        );
    ...
    public void user_is_logged_into_the_application() {
        // Login is already handled by Hook @Before
        // This step just confirms driver is active
        Assert.assertNotNull(b.driver, "❌ Driver is null - login failed");
    }
    ...
    public void user_should_be_redirected_to_login_page() {
        Assert.assertTrue(
            Pages.logoutPage.isLogoutSuccessful(),
